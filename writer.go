...
// running SetFrame/Close blocks that call, any other
// Start/Stop*/SetFrame/SetTicker/Close made concurrently on the same
// Writer, and Close itself. Write is unaffected.
//
//   - Start begins drawing immediately and on every tick of the Pair's
//     ticker (if any), until Stop/StopWith/StopNoClear/Close, or ctx is
//     cancelled. A nil ctx defaults to the Pair's own governing context.
//     A call while already running is a no-op returning nil. Fetches and
//     draws the first frame synchronously before returning.
...
//     implements io.Closer. Start/Stop/StopWith/StopNoClear/SetFrame all
...
//   - IsLive reports whether this writer is backed by a live spinner
...
// wrapped io.Writer. Start/Stop/StopWith/StopNoClear/SetFrame/SetTicker/
// Close are all no-ops, and Err returns an already-closed channel.
// WrapFilePair, WrapOS, and JustStart fall back to this automatically for
// any stream that isn't a real terminal, so a spinner never corrupts
...
func (WriterPassthrough) IsLive() bool { return false }
...
func (sw writerReal) SetTicker(ticker <-chan time.Time) error {
	return sw.st.setTicker(ticker)
}

func (sw writerReal) IsLive() bool { return true }
